...
- [x] **#5 Finder double-click** — opening a `.md` file via Finder double-click lands directly in the rendered reading view, equivalent to the File > Open path that already shipped. *Wireup shipped (`fileAssociations` + `RunEvent::Opened` + frontend listener); runtime smoke gated on `bundle.active = true` (slice 16 / #12 deferred).*
- [x] **#7 Dirty indicator + Cmd+S save** — any unsaved edit shows a visible dirty indicator; Cmd+S writes the file in place and clears the indicator on success. The dirty indicator stays sticky on failure. `save_md_file` is a custom Hashly Tauri command reached through the existing `core:default` IPC channel (no per-command ACL entry); path validation happens server-side in Rust via #44. `@tauri-apps/plugin-fs` is intentionally not used. *Save-failure blocking error dialog with "Save As…" escape deferred to a follow-up paired with #50's `dialog:allow-message` permission (filed as #56); for now, save-failure surfaces via `console.error` + sticky dirty.*
- [x] **#8 Unsaved-changes-on-close dialog** — closing a window with unsaved edits triggers a standard *Save / Don't Save / Cancel* dialog. *Custom HTML alertdialog (Tauri 2 plugin-dialog only natively supports 2-button confirms); a11y polish — ESC dismiss, default focus, focus trap, native `<dialog>` migration — filed as #66.* The same dialog also gates OPEN paths (File > Open / Finder double-click / File > New From Template) so opening a different doc with unsaved edits prompts before discarding.
- [x] **#9 Light & dark mode** — both themes render correctly; the app strictly follows the OS theme. No in-app toggle, no settings UI. *Shipped mechanically via slice 11's palette inversion (`@media (prefers-color-scheme: dark)`).*
- [ ] **#12 Package & ship** — a downloadable unsigned `.dmg` is published as a GitHub release, *and* a Homebrew cask is published in a `homebrew-hashly` (or equivalent) tap. Signing/notarization is intentionally deferred until there is user-base signal that the $99/yr Apple Developer ID is justified. **Slice 16 is deferred out of this milestone PR per user (2026-05-01); ships in a v0.2.x point release after v0.2 merges.**
...
- [x] **Spec templates — File > New From Template** — a *File > New From Template* menu exposes three baked-in templates: **PRD**, **Vision**, and **Task spec**, modelled on the structures already in `specs/` (parallel to what the feature-interviewer skill produces). Picking a template opens a new unsaved buffer in **edit mode** with frontmatter pre-filled (`status: Draft`, `author: <system user>`, `date: <today>`) and the template body below. Templates are static — baked into the app binary, not user-customizable in v0.2.
- [x] **Frontmatter recognition** — a leading `---\n…\n---\n` block at byte offset 0 of the document is parsed as YAML frontmatter rather than rendered as a horizontal rule. **Reading view** shows a small key/value metadata panel above the rendered body. On save, the frontmatter block is re-emitted at the top of the file ahead of the editor-serialized body. *Edit-view shows the metadata panel above the body but does NOT yet expose the frontmatter as a fenced-code-style editable region; that AC is deferred to follow-up #69. The byte-equal round-trip works because the raw block is preserved verbatim through edit/save cycles. CRLF-encoded frontmatter detection (Windows-authored or git-converted `.md`) is filed as a v0.3 follow-up — current LF-only detection treats CRLF docs as having no frontmatter.*
- [x] **Frontmatter autopopulation on template-new** — `date` is filled with today's date in `YYYY-MM-DD`; `author` is filled with the current macOS system user (falls back to literal string `Author` if unavailable). No other autopopulation hooks in v0.2.
- [x] **Frontmatter round-trips losslessly** — a doc opened, viewed, toggled edit/read, saved, and re-opened produces a frontmatter block byte-equal to the original (modulo any user edits inside the block). Verified as part of the Milkdown-fidelity gate task and pinned in `src-tauri/tests/save_md_file.rs::save_round_trip_*` for the disk layer.
...
- [x] **Typography wired** — Fraunces (wordmark), Schibsted Grotesk (UI/body), JetBrains Mono (mono/code/annotations) declared as the brand stacks with system fallbacks (`Times New Roman` / `-apple-system` / `SF Mono`). *Self-hosted woff2 loading + Google Fonts CDN wire-up + bundle-delta measurement deferred to follow-up #67 — system fallbacks render correctly today.*
- [x] **Wordmark `#hashly`** — gold `#` prefix in Fraunces weight 700, ink `hashly` in Fraunces weight 600, baseline-aligned. Used in the app titlebar / header area and in any future external context (README badge, GitHub social card).
- [x] **App icon (canonical SVG sources)** — `MarkGeometric` exported to `src/brand/mark.svg`, `src/brand/favicon.svg`, and `src/brand/icon-primary.svg` per the brand-sheet geometry. *PNG rasterization + `cargo tauri icon` → `.icns` set generation + `tauri.conf.json bundle.icon` wireup deferred to follow-up #68 (rasterization tooling not present in CI; SVGs are the canonical source).* Surface treatment per the brand sheet:
  - **Primary icon:** gold (`#c9a24b`) background, ink (`#14201b`) glyph, *no separate accent stem* — on a gold background, the gold accent would disappear, so both verticals render in ink. Pinned in `src-tauri/tests/brand.rs::brand_icon_primary_svg_renders_ink_on_gold_with_no_accent_stem` (gold appears EXACTLY ONCE in the SVG — the bg).
...
- [x] **Favicon** — `MarkGeometric` rendered as `src/brand/favicon.svg` (paper background, ink glyph, gold accent), wired into `index.html` via `<link rel="icon" type="image/svg+xml">`. *PNG fallbacks (32×32 + 16×16) for older browsers / OS-level shortcut tile rendering deferred to follow-up #68.*
- [x] **Mark usage rules enforced** — codified in `src/brand/USAGE.md`:
...
- [x] **First-run paint shows brand identity, not unstyled defaults** — palette, wordmark, and favicon all install before first paint via the Vite-bundled CSS + HTML. *Full first-frame brand identity (Fraunces wordmark in custom typeface + `MarkGeometric` `.icns` Dock icon) requires the deferred font-loading (#67) and icon rasterization (#68); system fallbacks + favicon SVG cover the gap.*
